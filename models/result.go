...
func (u *Result) TableName() string {
	return "results"
}

// AddResult insert a new Result into database and returns
// last inserted Id on success.
func CreateResult(m *Result) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}
...
func DeleteResult(id int64) (err error) {
	o := orm.NewOrm()
	v := Result{Base: Base{Id: id}}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&Result{Base: Base{Id: id}}); err == nil {
